...
(() => {
  ...
  async function renderCatalog(){
    ...
    host.innerHTML = items.map(x => {
      ...
      const orderLink = `contacts.html?subject=${encodeURIComponent('Заказ: ' + x.title)}&format=${encodeURIComponent(x.title)}&slug=${encodeURIComponent(x.slug)}&price=${encodeURIComponent(x.price_from||'')}`;
      return `<article class="program-card card">
        <img loading="lazy" src="${x.image}" alt="${x.title} — изображение">
        <h3>${x.title}</h3>
        <p class="muted">${x.desc||''}</p>
        <div class="price">${price}</div>
        <p><a class="btn" href="${orderLink}">Заказать</a></p>
      </article>`;
    }).join('');
  }
  ...
  async function renderGallery(){
    ...
    host.innerHTML = rows.map(g => `
      <figure>
        <img loading="lazy" src="${g.path}" alt="${g.title||'Фото из галереи'}">
  ...
  function setupContacts(){
    ...
    form.addEventListener('submit', (e)=>{
      ...
      const mailto = `mailto:${state.cfg?.email || ''}?subject=${encodeURIComponent(subj)}&body=${encodeURIComponent(body)}`;
  ...
  async function setupWheel(){
    ...
    tracks.forEach(track => {
      ...
      items.forEach((src, i) => {
        ...
        el.innerHTML = `<img loading="lazy" src="${src}" alt="Аниматор ${i+1}">`;
